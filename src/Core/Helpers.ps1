...
function Move-PathToRecycleBin {
    ...
    try {
        $shell = if ($ShellComObject) { $ShellComObject } else { New-Object -ComObject Shell.Application -ErrorAction Start }
        ...
        return (-not (Test-PathExists $item.FullName))
    } catch {
        return $false
    }
}
...
function Remove-DirectoryContentsWithRecovery {
    ...
    if (-not (Test-PathExists $DirectoryPath)) {
